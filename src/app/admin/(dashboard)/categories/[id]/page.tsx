  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-2xl shadow-soft border border-slate-100 p-6">
        <SubmitForm action={updateCategory} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Category Name</label>
              <input type="text" name="name" defaultValue={category.name} required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0056b3] focus:bg-white transition-all text-sm" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Slug</label>
              <input type="text" name="slug" defaultValue={category.slug} required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0056b3] focus:bg-white transition-all text-sm" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mb-2">
               {category.imageUrl && <p className="text-xs text-slate-500">Current image uploaded.</p>}
            </div>
            {/* ImageUploader now supports defaultValue! */}
            <ImageUploader name="imageUrl" defaultValue={category.imageUrl || ""} />
          </div>

          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Description (Optional)</label>
            <RichTextEditor name="descriptionHtml" defaultValue={category.descriptionHtml || ""} placeholder="Write a description for this category..." />
          </div>
          ...
          <div>
            ...
            <input type="text" name="bgClass" defaultValue={category.bgClass || ""} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0056b3] focus:bg-white transition-all text-sm" />
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 flex gap-4">
            ...
            <Link href="/admin/categories" className="px-8 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 transition-colors">
              Cancel
            </Link>
          </div>
        </SubmitForm>
