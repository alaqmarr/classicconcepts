...
export function CategoryCard({ category }: CategoryCardProps) {
  const bgClass = category.bgClass || "bg-slate-100";

  return (
    <Link href={`/shop/c/${category.slug}`} className="group block">
        ...
        <div className={`w-full h-52 ${bgClass} flex items-center justify-center relative overflow-hidden`}>
          ...
          {category.imageUrl ? (
            ...
            <div className="w-24 h-24 bg-white/40 backdrop-blur-sm rounded-xl border border-white/60 shadow-sm group-hover:scale-110 transition-transform duration-500 flex items-center justify-center">
              <span className="text-3xl font-extrabold text-white/50">{category.name.charAt(0)}</span>
            </div>
          )}
        </div>
        
        {/* Card Footer */}
        <div className="flex-1 flex items-center justify-between px-6 py-4 bg-white z-20 border-t border-slate-50 relative">
          <h3 className="font-bold text-[#0056b3] group-hover:text-slate-900 transition-colors">{category.name}</h3>
